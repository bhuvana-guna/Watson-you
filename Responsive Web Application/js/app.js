...
.controller('NavCtrl', function($scope, $ionicSideMenuDelegate, $http, $state) {
    ...
    //function(url, title, descr, image, winWidth, winHeight) {
      ...
      //winhttp://m.facebook.com/sharer.php?u='+url,{openExternal : true});
      window.open('http://facebook.com/sharer.php?u='+url,'_blank');
    ...
    $scope.tweet=function(id){
       ...
       // navigator.app.loadUrl("http://twitter.com/intent/tweet?text="+title, {openExternal : true});
        window.open('http://twitter.com/intent/tweet?text='+title,'_blank');
...
if(window.localStorage['allConceptDetails'] == undefined){
    var url = "http://newsintagsv2.eu-gb.mybluemix.net/GetAllConcepts?callback=JSON_CALLBACK";
    $http.jsonp(url)
      .success(function(data){
          console.log(d=data);
          
          $scope.allConceptDetails =data.conceptDetails;
          window.localStorage['allConceptDetails'] = JSON.stringify($scope.allConceptDetails);
console.log(a=data.conceptDetails);
         airlines = d.conceptDetails;
         airlines = airlines.sort(function(a, b) {
      ...
      $scope.allConceptDetails= JSON.parse(window.localStorage['allConceptDetails']);
    }
 ...
 if(window.localStorage['similar_'+newsId] == undefined){
        $http.jsonp(url)
         ...
         /* var url = "http://newsintagsv2.eu-gb.mybluemix.net/AddConceptToWishlist?likedConcepts"+concepts+"&userName="+screenName;
          console.log(url);
           $http.jsonp(url)
           .success(function(data){
                  console.log(d=data);
...
document.getElementsByClassName("left-menu")[0].classList.add("hide");
document.getElementsByClassName("menu-content")[0].classList.add("full-page");
    ...
    $rootScope.hide_header = true;
      ...
      $scope.signIn = function(form) {
        if(form.$valid) {
           ...
           document.getElementsByClassName("left-menu")[0].classList.remove("hide");
           document.getElementsByClassName("menu-content")[0].classList.remove("full-page");
            $state.go('tabs.trending');
        }
      };  
